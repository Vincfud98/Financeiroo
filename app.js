...
document.querySelector("#wishlistForm").addEventListener("submit", async (event) => {
  event.preventDefault();
  const form = event.currentTarget;
  const values = Object.fromEntries(new FormData(form).entries());
  const existing = data.wishlist.find((item) => item.id === values.id);
  const photoFile = form.elements.photoFile.files[0];
  const uploadedPhoto = photoFile ? await readImageFile(photoFile) : "";
  const wishlistItem = {
    ...
    photo: uploadedPhoto || values.photo.trim() || existing?.photo || "",
  };
  if (existing) {
